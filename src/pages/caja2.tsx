...
const ComprasPagos: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('https://190.102.142.247:82/api/compras/obtener');
  ...
  const handleAprobado = async () => {
    ...
    try {
      ...
      await axios.post('https://190.102.142.247:82/api/compras/aprobar', null, {
        params: {
      ...
      <Modal open={modalOpen} onClose={handleCloseModal}>
        <Box
          sx={{
            ...
            width: '95%', // Aumentar el ancho del modal
              ...
              <Grid item xs={12} md={6}>
                {selectedCompra.imagenBase64 ? (
                  <img
                    ...
                    alt="Voucher"
                    style={{
                      width: '280px', // Asegura que la imagen ocupe todo el ancho del contenedor
                      height: 'auto', // Mantener proporción de la imagen
                      maxHeight: '500px', // Limitar la altura máxima de la imagen
                      borderRadius: '8px',
                    }}
                  />
                ) : (
